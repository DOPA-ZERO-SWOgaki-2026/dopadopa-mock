...
///
/// SW_ogaki プロジェクトの実装をそのまま移植したもの。iOS のサードパーティアプリには
/// 「画面が消灯した」ことを直接知らせる公開 API は無いため、実用上もっとも近い代替として
/// `UIApplication` の `protectedDataWillBecomeUnavailable` / `protectedDataDidBecomeAvailable`
/// 通知（＝デバイスがロックされる／ロック解除される瞬間）を利用して自動計測する。
/// DopaDopa アプリではこの累積時間を「デジタルデトックスタイム」として扱い、
/// 1 分ごとに 1 ポイントを付与する。
///
...
final class ScreenOffTracker: ObservableObject {
    ...
    /// 現在の合計（確定分 + 進行中の分）。DopaDopa のポイント計算・表示はこれを基準にする。
    var combinedSeconds: TimeInterval { totalScreenOffSeconds + currentOffSeconds }
